...
# 2124 bytes = (24*2)+4+(256*2)+(256*2)+4+4+4+4+4+4+4+(100*2)+4+4+4+(800*1)+4+2+2
PAC_HEADER_FMT = '48s I 512s 512s I I I I I I I 200s I I I 800s I H H'

# 2580 byte = 4+(256*2)+(256*2)+(256*2)+4+4+4+4+4+4+(5*4)+(996*1)
...
def verifyCRC16(f, ph, debug):
    ...
    bufsize = 64 * 1024
    tempsize = ph['dwSize'] - struct.calcsize(PAC_HEADER_FMT)
    tsize = tempsize
    crc2val = 0
    while tempsize > 0:
        if tempsize < bufsize:
...
def main(pacfile, outdir=None, debug=False, checkCRC16=False):
    ...
    if outdir == None:  # use 'outdir' as default output directory if None specified
